import { BookingTypes, GuestTypes } from '@/types';
...
export async function updateGuest(id: number, updatedFields: GuestTypes) {
  const { data, error } = await supabase
    .from('guests')
...
export async function updateBooking(id: number, updatedFields: BookingTypes) {
  const { data, error } = await supabase
    .from('bookings')
    .update(updatedFields)
    .eq('id', id)
